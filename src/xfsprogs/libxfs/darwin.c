...
{
	/*
	 * Use HW_MEMSIZE instead of HW_PHYSMEM for 64-bit memory size.
	 * HW_PHYSMEM returns a 32-bit value which is truncated on systems
	 * with more than 4GB of RAM. HW_MEMSIZE returns the full 64-bit value.
	 */
	uint64_t	physmem;
	...
	return (unsigned long)(physmem >> 10);
}
