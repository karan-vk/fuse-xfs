 ...
 *
 * V5 filesystems with the FTYPE feature store file type information
 * in directory entries, adding a byte after the name.
 */
...
typedef struct xfs_dir2_sf_entry {
	...
	/*
	 * A single byte containing the file type field follows the name
	 * if the filesystem has the FTYPE feature enabled.
	 * Then the inode number follows.
	 */
	/* __uint8_t		ftype; */	/* file type (if ftype enabled) */
	/* xfs_dir2_inou_t	inumber; */	/* inode number, var. offset */
} __arch_pack xfs_dir2_sf_entry_t;

/*
 * File type values for directory entries.
 * These match the DT_* values from readdir.
 */
...
/*
 * Convert between inode mode and directory file type.
 */
static inline __uint8_t xfs_mode_to_ftype(int mode)
{
	switch (mode & S_IFMT) {
	case S_IFREG:
...
/*
 * Get pointer to inode number in shortform entry.
 * For FTYPE filesystems, the ftype byte is between name and inumber.
 */
...
/*
 * Get pointer to inode number for ftype-enabled filesystem
 */
static inline xfs_dir2_inou_t *
xfs_dir3_sf_inumberp(xfs_dir2_sf_entry_t *sfep)
{
	return (xfs_dir2_inou_t *)&(sfep)->name[(sfep)->namelen + 1];
}

/*
 * Get file type from shortform entry (ftype-enabled filesystem)
 */
static inline __uint8_t
xfs_dir3_sf_get_ftype(xfs_dir2_sf_entry_t *sfep)
{
	return sfep->name[sfep->namelen];
}

/*
 * Set file type in shortform entry (ftype-enabled filesystem)
 */
static inline void
xfs_dir3_sf_put_ftype(xfs_dir2_sf_entry_t *sfep, __uint8_t ftype)
{
...
/*
 * Entry size calculation for FTYPE-enabled filesystems.
 * These include the extra ftype byte.
 */
